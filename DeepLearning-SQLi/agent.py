"""
Reinforcement learning agent that uses Q learning to exploit a SQLi vuln
"""
...
class Agent():
    ...
    def _select_action(self):
        """
        Chooses one action from a pre generated list of actions 
        Action is chosen based on the Q-table

        If the deterministic option is not set then the agent will sometimes choose random actions
        """
    ...
    def step(self):
        """
        Takes one step within an episode
        Selects an action, calls the step function in the environment,
    ...
    def run_episode(self, vuln_type):
        """
        Start an episode
        vuln_type determines what kind of
        vulnerability the environment should be set to
        #1 = Stack based
        #2 = Union based
        #3 = Stack based + input filter
        #4 = Union based + input filter
    ...
    def _update_state(self, action_nr, response_interpretation):
        """
        Updates the state of the reinforcement learning agent
